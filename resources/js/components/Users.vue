<template>
  <div class="container">
    ...
    <div class="row mt-7" v-if="!$gate.isAdmin()">
      <not-found></not-found>
    </div>
    <!-- Modal -->
    <div
...
export default {
  ...
  methods: {
    ...
    createUser() {
      ...
      //EMIT EVENT AFTER CREATING A USER
      Fire.$emit("afterCreatingUser");
  ...
  created() {
    ...
    Fire.$on('searching', () => {
       ...
       axios.get('api/findUser?q='+query).then((result) => {
         this.users = result.data;
       }).catch((error) => {

       })
    });
    Fire.$on("afterCreatingUser", () => {
      this.loadUsers();
    });
  },
};
